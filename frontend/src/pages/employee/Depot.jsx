...
const Depot = () => {
  ...
  useEffect(()=>{
    const fetchRayons = async ()=>{
        const response = await fetch(API_BACKEND +'/rayon/'+user.depot._id , {
...
const handleDeleteRayonSubmit = async(e)=>{
  ...
  return (
    ...
    <BreadCrumb navigations={{id : 1 , label : "Depot" , lien : "/employee/depot"}} titre={"Depot: " + user.depot.name}   />
            ...
            { rayons &&  rayons.map((item, index) => (
                    <div key={index} className="p-2 sm:w-1/2 w-full">
                        ...
                        <div className="flex items-center">
                            ...
                            <Link to={'/employee/rayon/' + item._id + "/"+ item.name}>{item.name}</Link>
                            </span>
                        </div>
